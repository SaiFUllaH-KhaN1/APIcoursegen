...
### Shadows repair prompts are found only in PROMPTS file since all the instruction prompts are equal for both PROMPTS and PROMPTS_WITHOUT_FILE.
### Only the shadow repair prompts are used to generate edges array. All other prompts use a point 4 in the "What to do" section of prompts to neglect producing the edges array. 
### The language prompt for JSON generation prompts is different than the language prompt for setups becuase setups don't generate json keys and values.
...
prompt_linear = PromptTemplate(
    input_variables=["input_documents","human_input","content_areas","learning_obj","language","mpv","mpv_string"],
    template="""
...
prompt_linear_shadow_edges = PromptTemplate(
    ...
    and the Edges Array defines the interconnectivity between the Node Blocks via their unique IDs.

    Your task is to regenerate the edges array and it should be such that the interconnection of node ids are in accordance with the rules
    set forth in the practical examples in the Instruction Set. You can semantically analyze and decide how node ids be interconnected with each other.

    You are free to ignore those nodes id's inclusion in edges array which does not logically make sense to be included. When you ignore these node ids and
    not mention in edges array, I will come to know about them and I will delete them myself.

    !!!WARNING: YOU ONLY AND ONLY GIVE YOUR RESPONSE THAT HAS EDGES ARRAY AND NOTHING ELSE. GIVE A JSON PARSEABLE EDGES ARRAY AS YOUR RESPONSE.
...
"edges":
  [
    ...
    BELOW IS THE INSTRUCTION SET BASED ON WHICH THE 'OUTPUT' WAS CREATED ORIGINALLY:
    Instruction Set:
    [[[
...
prompt_branched_setup = PromptTemplate(
    ...
    You are an educational bot designed to process information based on the user's requirements defined by 'Human Input', 'Input Documents', 
    'Learning Objectives', and 'Content Areas'. Using this context, your task is to analyze the Input Documents and organize their content into a meta-data 
    structure, segregated by subtopics. Each subtopic is derived from the main subject as specified by the Human Input and aligned with the 
    Learning Objectives and Content Areas. The number of subtopic is directly related to the number of Learning Objectives.
    For example 5 Learning Objectives will produce 5 subtopics. For each subtopic, you extract and categorize relevant information from the Input Documents, ensuring the meta-data 
    ...
    'Input Documents' is the verified source of information. You absolutely limited to the knowledge present in the 'Input Documents'
    and are not aware of any other knowledge besides it.
    ...
    and upto Subtopic X (X being the number) that you creatively deem suitable to include according to the data in 'Input Documents' only...
...
prompt_branched_setup_continue = PromptTemplate(
    ...
    You respond in the language of "{language}", since your responses are given to {language} speakers and they can only understand the language of {language}.
    You are an educational bot designed to process information based on the user's requirements defined by 'Human Input', 'Input Documents', 
    and 'Learning Objectives'. Using this context, your task is to analyze the Input Documents and organize their content into a meta-data 
    structure, segregated by subtopics. Each subtopic is derived from the main subject as specified by the Human Input and aligned with the 
    Learning Objectives. The number of subtopic is directly related to the number of Learning Objectives.
    For example 5 Learning Objectives will produce 5 subtopics. For each subtopic, you extract and categorize relevant information from the Input Documents, ensuring the meta-data 
    reflects a clear and organized breakdown of the content to address the user's needs.
    For each of the subtopic that contributes to the main subject, you create a detailed information-database of every possible information available
    using the Input Parameters. 
...
prompt_simulation_pedagogy_setup = PromptTemplate(
    ...
    "Input Documents", the branching points should ask students to perform steps that lead to successful CPR outcomes, with each choice 
    ...
    on detailed techical steps involved, reflecting the actual consequences of correct or incorrect or partially correct actions as outlined in the "Input Documents". 
    ...
    Ensure that you cover all the subject matters given to you in the 'Input Documents' according to 'Human Input'
    in your simulation story.
...
prompt_LO_CA_GEMINI = PromptTemplate(
    ...
    Keep the list of LearningObjectives and ContentAreas brief, concise, short, and specific to the 'Human Input' requirement and its relevant 'Input Docuemnts' information.
